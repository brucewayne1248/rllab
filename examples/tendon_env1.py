...
#from rllab.utils.plot_utils import Arrow3D, mypause
...
#      self.action_dim = 3 # number of actions per time step
   ...
   def reset(self):
      ...
      self.l1 = 0.5 * (self.lmin+self.lmax)
      self.l2 = 0.5 * (self.lmin+self.lmax)
      self.l3 = 0.5 * (self.lmin+self.lmax)
   ...
   def step(self, action):
      ...
      reward = -1 -5000*(new_dist_euclid-old_dist_euclid)
      ...
      if norm(self.tip_vec-self.goal) < self.eps:
         reward = 0.5*float(self.max_steps)
         done = True
         self.dist_end = norm(self.goal-self.tip_vec)
   ...
   def render(self, pause=0.0000001, frame="bishop", save_frames=False):
      ...
      if self.steps % 5 == 0:
         print("STEP{:3d}\tDISTANCE: {}mm".format(self.steps, norm(self.goal-self.tip_vec)))
