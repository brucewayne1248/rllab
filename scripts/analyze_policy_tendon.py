...
def render_episode(env, tendon_lengths, goal, tangent_vec_goal, save_frames=False):
   env.reset()
   env.goal = goal
   ...
   for i in range(len(tendon_lengths)):
      ...
      env.render(mode="human", save_frames=save_frames)

   alpha = env.get_diff_angle(degree=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_frames', type=int, default=0,
                        help='0 to not save frames, 1 to save frames during rendering')
